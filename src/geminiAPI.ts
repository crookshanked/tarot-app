...
export async function callGeminiAPI(prompt: string, apiKey: string): Promise<string | null> {
    ...
    const MODEL_NAME = (constants.modelSelect as HTMLSelectElement)?.value; // Get model from select input
    const API_BASE_URL = (constants.providerSelect as HTMLSelectElement)?.value; // Get base input URL from select input
...
export async function getCardMeaning(cardName: string, isReversed: boolean, elementId: string): Promise<void> {
    ...
    const apiKey = (constants.apiKeyInput as HTMLInputElement).value; // Assert type as HTMLInputElement
    const meaningTextarea = document.getElementById(elementId) as HTMLTextAreaElement | null; // Assert type as HTMLTextAreaElement or null
...
export async function getInterpretation(cards: TarotCard[]): Promise<void> {
    // **GEMINI API CALL for overall interpretation**
    const queryInput = document.getElementById('query') as HTMLInputElement | null; // Assert type as HTMLInputElement or null
    ...
    const interpretationTextarea = document.getElementById('interpretation') as HTMLTextAreaElement | null; // Assert type as HTMLTextAreaElement or null
    ...
    const downloadLink = constants.downloadLink as HTMLAnchorElement | null; // Assert type as HTMLAnchorElement or null
